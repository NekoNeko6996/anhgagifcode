...
public interface GiftAccountJpaRepository extends JpaRepository<GiftAccounts, String> {
    ...
    @Query(value = "SELECT g.* FROM gift_accounts g JOIN pool_account_mappings p ON g.id = p.account_id WHERE p.pool_id = :poolId AND g.status = 'AVAILABLE' LIMIT 1 FOR UPDATE SKIP LOCKED", nativeQuery = true)
